...
mongoimport -h ds145790.mlab.com:45790 -d mongolabfree -c stops -u steve -p felisa --file stops.json

mongoimport -h ds163679.mlab.com:63679 -d heroku_5d4vj37d -c allsigns -u steve -p modernwater360 --file allsigns.json

heroku_5d4vj37d 
ndjson-filter < pleiades-places-20180428.json["@graph"].features 'd.properties.B == "S"' | ndjson-map  '{"type": "Feature", "properties": {"OID": d.properties.ID, "MUT": d.properties.MUT,"geometry": {"type": "Point", "coordinates": d.geometry.coordinates}}}' > signDataS.ndjson

gawk < pleiades-places-20180428.json '{gsub("SANITATION||BROOM||NO||PARKING||NIGHT||REGULATION||STARS||STAR||SYMBOLS||SYMBOL||HALF||MOON||SINGLE||ARROW", "");print}' > trim1.ndjson
mongoimport -h ds259079.mlab.com:59079 -d heroku_7rt6qd22 -c pleiades -u steve -p modernwater360 --file pleiades-places-20180428.json



mongoexport -h ds259079.mlab.com:59079 -d heroku_7rt6qd22 -c pl -u steve -p modernWater360 -o pldb.json




ndjson-map < pldb.json '{"type": "Feature", "properties": {"Authors": d.authors, "ConnectsWith": d.connectsWith, "Description": d.description, "Distance": d.distance, "FType": d.featureTypes, "Geocontext": d.geoContext, "ConnectsWith": d.hasConnectionWith, "Id": d.id, "LocationPrecision": d.LocationPrecision, "MaxDate": d.maxDate, "MinDate": d.minDate, "Path":d.path, "RepLat": d.reprLat, "RepLng": d.repLong, "Tags": d.tags, "TimePeriods": d.timePeriods, "TimPeriodKeys": d.timePeriodsKeys, "Title": d.title}, "geometry": {"type": "Point", "coordinates": d.geometry.coordinates}}' > geoPleiades.ndjson


ndjson-map < graawcvt.json '{"type": "Feature", "properties": {"name": d.NAME, "modernName": d.NAME_MOD, "auth_anc": d.AUTH_ANC, "auth_mod":d.AUTH_MOD, "country": d.COUNTRY, "am": d.AM, "pp": d.PP, "re": d.RE, "bw": d.BW, "qu": d.QU, "mo": d.MO, "cn": d.CN, "sl": d.SL, "sh": d.SH, "ph": d.PH, "co": d.CO},  "geometry": {"type": d.geometry.type, "coordinates": d.geometry.coordinates}}' > graawcvt.ndjson
ndjson-map < hoards.json '{"type": "Feature", "properties": {"lable": d.label, "id": d.id, "when": d.when},  "geometry": {"type": d.geometry.type, "coordinates": d.geometry.coordinates}}' > geohoards.ndjson
